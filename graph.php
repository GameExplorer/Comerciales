...
// Fetch users from the users database
include 'conexion_users.php';

$sqlUsers = "SELECT Username FROM users_table";
$stmtUsers = sqlsrv_query($connUsers, $sqlUsers);
...
<body>
    ...
    <script>
        ...
        <?php if ($userRole == 'boss') : ?>
            users.forEach(user => {
                const userData = monthlyData.filter(data => data.Usuario === user);
                const userFacturado = userData.map(data => data.Facturado);
                datasets.push({
                    label: user,
                    data: userFacturado,
        ...
        <?php else : ?>
            datasets.push({
                label: 'Facturado',
                data: monthlyData.map(data => data.Facturado),
